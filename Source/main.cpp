...
#define AS currentGame->_hightOrbit[n]
...
class gameInfo
{
	public:
		...
		// establish player vector.
		// from this point on players are contaned in _lowOrbit.
		vector<_Tank*> _lowOrbit;
		
		// establish bullet / shell vector.
		// from this point on bullets / shells are contaned in _highOrbit.
        ...
        if( PeekMessage( &msg, NULL, 0, 0, PM_REMOVE ) )
        {
			...
			else if (msg.message == WM_KEYDOWN)
			{
				...
				for (int n = 0; n < TOTAL_PLAYERS; n++)
				{
					...
					if (_KeyBox == currentGame->_lowOrbit[n]->_Tank_Key_ID._Fire_ID)
					{
						currentGame->_lowOrbit[n]->_Key_fire = true;
					}
				}
			}
			...
			else if (msg.message == WM_KEYUP)
			{
				...
				for (int n = 0; n < TOTAL_PLAYERS; n++)
				{
					...
					if (_KeyBox == currentGame->_lowOrbit[n]->_Tank_Key_ID._Fire_ID)
					{
						currentGame->_lowOrbit[n]->_Key_fire = false;
					}
				}
			}
            else 
...
void updateGame(HDC deviceContext, gameInfo* currentGame)
{
	...
	for (int n = 0; n < TOTAL_PLAYERS; n++)
	{
		...
		*/
		...
		AP->_Turret_Direction += (AP->_Key_TRot_Right - AP->_Key_TRot_Left) * TANK_T_ROT_SPEED;
		...
		if (AP->_Key_fire)
		{
			if (AP->_Turret_CoolDown == 0)
			{
				_Shell* $Shell = new _Shell;

				$Shell->_pos._x = AP->_pos._x + (sin(AP->_Turret_Direction * (M_PI / 180)) * 0.5f);
				$Shell->_pos._y = AP->_pos._y + (cos(AP->_Turret_Direction * (M_PI / 180)) * 0.5f);
				$Shell->_Direction = AP->_Turret_Direction * (M_PI / 180);

				currentGame->_hightOrbit.push_back($Shell);
				AP->_Turret_CoolDown = 500;
			}
		}

		if (AP->_Turret_CoolDown > 0)
		{
			AP->_Turret_CoolDown -= 1;
		}
	}

	for (int n = 0; n < currentGame->_hightOrbit.size(); n++)
	{
		float $Xpos = (sin(AS->_Direction) * (AS->_Speed * deltaAdjustment));
		float $Ypos = (cos(AS->_Direction) * (AS->_Speed * deltaAdjustment));
		AS->_pos._x += $Xpos;
		AS->_pos._y += $Ypos;

		if (abs(AS->_pos._x) > TANK_X_CONSTRAINT)
		{
			AS->_pos._x = (abs(AS->_pos._x) / AS->_pos._x) * TANK_X_CONSTRAINT * -1;
		}

		if (abs(AS->_pos._y) > TANK_Y_CONSTRAINT)
		{
			AS->_pos._y = (abs(AS->_pos._y) / AS->_pos._y) * TANK_Y_CONSTRAINT * -1;
		}
	}
}
...
void draw(HDC deviceContext, gameInfo* currentGame)   
{
	...
	for (int n = 0; n < TOTAL_PLAYERS; n++)
	{
		...
		glPushMatrix();														// save the current matrix state, in case anything important was done before we got here
			...
			// then rotate to the current player's rotation around z axis
			glRotatef(-currentGame->_lowOrbit[n]->_Direction * (180 / M_PI), 0.0f, 0.0f, 1.0f);
		...
		glTranslatef(AP->_pos._x, AP->_pos._y, -10.0);
		glRotatef(-AP->_Turret_Direction, 0.0, 0.0, 1.0);
		glDrawArrays(GL_TRIANGLES, 0, 3);
	...
	for (int n = 0; n < currentGame->_hightOrbit.size(); n++)
	{
		glVertexPointer(3, GL_FLOAT, 0, shellVertexArray);
		glColorPointer(4, GL_FLOAT, 0, shellColorArray);

		glPushMatrix();

		glLoadIdentity();
		glTranslatef(currentGame->_hightOrbit[n]->_pos._x, currentGame->_hightOrbit[n]->_pos._y, -10.0f);

		glRotatef(currentGame->_hightOrbit[n]->_Direction * (180 / M_PI), 0.0f, 0.0f, 1.0f);
...
gameInfo::gameInfo()
{
	...
	for (int n = 0; n < TOTAL_PLAYERS; n++)
	{
		...
		case 0:
			...
			_lowOrbit[0]->_Tank_Key_ID._Fire_ID = 'R';
			break;
		...
		case 1:
			...
			_lowOrbit[1]->_Tank_Key_ID._Fire_ID = 'P';
			break;
